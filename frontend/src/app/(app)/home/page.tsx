...
type AttentionItem = {
  id: string;
  type: "overdue-task" | "upcoming-task" | "closing-deal";
  title: string;
  companyId: string;
  companyName: string;
  detail: string;
...
function buildAttentionItems(data: DashboardData): AttentionItem[] {
  ...
  for (const t of data.tasks) {
    ...
    if (isPast(due)) {
      items.push({
        ...
        companyName: t.company?.name || "Untitled",
  ...
  for (const d of data.deals ?? []) {
    if (d.stage?.isWon || d.stage?.isLost || !d.expectedCloseDate) continue;
...
const ATTENTION_STYLE = {
  "overdue-task": "bg-red-100 text-red-600 dark:bg-red-950 dark:text-red-400",
  "upcoming-task": "bg-amber-100 text-amber-600 dark:bg-amber-950 dark:text-amber-400",
...
export default function HomePage() {
  ...
  async function handleToggle(t: TaskWithCompany) {
    try {
      if (!t.companyId) return;
      await updateTask(t.companyId, t.id, {
  ...
  const totalCompanies = data?.stats.total ?? 0;
  ...
  const openDeals = data?.deals?.filter((d) => !d.stage?.isWon && !d.stage?.isLost) ?? [];
      ...
      <div className="border-b px-6 py-4">
        ...
        <div className="mt-2 flex flex-wrap items-center gap-x-6 gap-y-1 text-sm">
          <Link href="/customers" className="flex items-center gap-1.5 text-muted-foreground hover:text-foreground transition-colors">
            ...
            <span className="font-medium tabular-nums text-foreground">{totalCompanies}</span>
            companies
          </Link>
                    ...
                    >
                      {item.task && (
                        <Checkbox
                          className="shrink-0"
                          checked={false}
                          onCheckedChange={() => handleToggle(item.task!)}
                        />
                      )}
                      <div
                        className={cn(
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                          ...
                          <span className={cn(item.type === "overdue-task" && "text-destructive")}>
                            {item.detail}
                          </span>
                        </div>
                      </div>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <section>
              ...
              {events.length === 0 ? (
                <div className="rounded-lg border border-dashed bg-muted/30 px-4 py-8 text-center">
                  <p className="text-sm text-muted-foreground">
                    No activity yet. Events appear as you work with companies.
                  </p>
                </div>
              ) : (
